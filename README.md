...
*(엔진의 핵심 모듈과 서브시스템이 연결된 구조 다이어그램 삽입)*

엔진 코어가 중심에서 각 서브시스템(렌더링, 물리, 애니메이션, 사운드 등)을 프레임 단위로 관리하며 상호작용하는 구조로 설계되었습니다.
...
- 일정한 게임 속도 유지 및 연출적 효과 적용 가능

## 🚀 절두체 컬링 & 옥트리 최적화
- 뷰 절두체 밖의 객체 렌더링 제외로 GPU 부하 감소
- 씬을 옥트리 구조로 나눠 효율적인 컬링을 통해 성능 향상

## 🎯 충돌 시스템 (OBB/AABB/Sphere)
- 객체의 특성에 따라 충돌 형태를 OBB, AABB, Sphere로 구분하여 정확하고 효율적인 충돌 처리
- SAT(Separating Axis Theorem) 기반의 정밀 충돌 검사 구현

## ⚙️ 레이어 인덱스 기반 충돌 최적화
- 객체 간 충돌 그룹을 레이어로 관리하여 불필요한 충돌 검사를 사전에 제외
- 충돌 매트릭스를 통한 검사 최적화로 퍼포먼스 개선

## 💡 디퍼드 렌더링 및 HLSL 효과
- 디퍼드 렌더링으로 광원 개수에 상관없이 효율적인 조명 연산
- Rim Light, Glow, Blur, 블럭 노이즈 등의 후처리 효과를 HLSL 셰이더로 구현하여 다양한 시각 효과 제공

## 🌐 Nvidia PhysX 물리 연동 및 캐릭터 컨트롤러
- PhysX를 통한 강체 시뮬레이션 및 충돌 처리
- PhysX의 Character Controller로 캐릭터의 안정적이고 현실적인 이동 구현

## 🔊 FMOD 기반 사운드 시스템
- FMOD를 통해 다양한 오디오 포맷과 3D 사운드 지원
- 이벤트 기반의 사운드 관리 및 DSP 필터를 통한 풍부한 오디오 효과 구현
