...
void CMesh::Cooking_TriangleMesh()
{
	...
	m_pTriangleMesh = CGameInstance::Get_Instance()->Get_PhysX_Physics()->createTriangleMesh(readBuffer);
}

HRESULT CMesh::Initialize_Prototype(MODEL_TYPE eModelType, const aiMesh* pAIMesh, vector<class CBone*>& Bones, _fmatrix TransformMatrix)
{
...
HRESULT CMesh::Initialize_Prototype(MODEL_TYPE eModelType, shared_ptr<MESH_DATA> pAIMesh, vector<class CBone*>& Bones, _fmatrix TransformMatrix)
{
...
#pragma region INDEX_BUFFER
	...
	//for (unsigned int i = 0; i < pAIMesh->iNumFaces; ++i) {
	//	const FACEINDICES32& face = pAIMesh->pIndices[i];

	//	if (face.numIndices != 3) continue; // 삼각형이 아닌 페이스는 무시
	//	for (unsigned int j = 0; j < face.numIndices; ++j) {
	//		indices.push_back(face.pIndices[j]);
	//	}
	//}
...
HRESULT CMesh::Ready_VIBuffer_For_Anim(shared_ptr<MESH_DATA> pAIMesh, vector<class CBone*>& Bones)
{
	m_iVertexStride = sizeof(VTXANIMMESH);
...
CMesh* CMesh::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, MODEL_TYPE eModelType, const aiMesh* pAIMesh, vector<class CBone*>& Bones, _fmatrix TransformMatrix)
{
 ...
 CMesh* CMesh::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, MODEL_TYPE eModelType, shared_ptr<MESH_DATA> pAIMesh, vector<class CBone*>& Bones, _fmatrix TransformMatrix)
{
	CMesh* pInstance = new CMesh(pDevice, pContext);
