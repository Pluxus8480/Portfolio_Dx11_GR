...
HRESULT CAnimation::Initialize(shared_ptr<ANIMATION_DATA> pAIAnimation, const vector<class CBone*>& Bones)
{
...
void CAnimation::Update_Transformation_Aperture(const vector<class CBone*>& Bones)
{
	for (size_t i = 0; i < m_iNumChannels; i++)
		m_Channels[i]->Update_Linear_Aperture(m_fCurrentTrackPosition, Bones, &m_CurrentKeyFrameIndices[i], m_fAperture);
}
...
CAnimation* CAnimation::Create(shared_ptr<ANIMATION_DATA> pAnimation, const vector<class CBone*>& Bones)
{
	CAnimation* pInstance = new CAnimation();
